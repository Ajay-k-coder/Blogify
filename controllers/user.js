...
async function userSignUpHandler(req, res, next) {
    ...
    const q =
        "INSERT INTO users(full_name, age, email, password, bio, profile_image_url, id)  VALUES (?)";
    ...
    bcrypt.genSalt(saltRounds, function (err, salt) {
        bcrypt.hash(body.password, salt, async function (err, hash) {
            ...
            try {
                const [results, fields] = await mysqlPool.query(q, [
                    arrayValue,
                ]);
            } catch (err) {
                console.log(err);
    ...
    async function main() {
        ...
        const htmlData = await ejs.renderFile(templetePath, {
            otp: generatedOtp,
        });
